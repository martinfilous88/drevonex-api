...
/* ── Cílové kategorie – stavební řezivo, hranoly, BSH, konstrukční prvky, OSB ── */
...
async function login() {
  ...
  try {
    // 0) nejdřív navštívit homepage — JAF někdy vyžaduje session cookie
    const home = await jfetch("https://www.jafholz.cz/");
    await home.text();
    console.log("[jaf] homepage navštíveno, cookies nastaveny");
    await sleep(800);
    ...
    // debug: uložit prvních 500 znaků
    if (html.length < 5000) {
    ...
    if (!form.length) {
      // zkusíme najít jakýkoliv formulář pro debug
    ...
    try {
      ...
      $("a[href]").each((_, a) => {
        ...
        if (pm && !products.has(pm[1]) && products.size < MAX_PRODUCTS) {
          ...
          // OSB produkty projdou vždy (i když obsahují "deska"); ostatní musí projít KEYWORDS+BLACKLIST
          const isOsb = /\bosb\b/i.test(name);
          if (isOsb || (KEYWORDS.test(name) && !BLACKLIST.test(name))) {
            products.set(pm[1], {
              id: `p${pm[1]}`,
              name: name,
              url: abs,
...
/* ── Statické produkty z JSON ── */
let cache = { updatedAt: null, loggedIn: false, items: [] };
const CATEGORY_MARGIN = {
  "OSB desky": 0.20,
  "Stavební řezivo": 0.20,
  "KVH masivní konstrukční hranoly": 0.20,
  "BSH lepené vrstvené hranoly": 0.18,
  "DUO/TRIO dřevěné hranoly": 0.18,
};
function sellPriceRaw(p) {
...
function loadStaticProducts() {
  try {
    ...
    console.warn("[jaf] static_products.json nenalezen, používám prázdnou cache");
  }
}
loadStaticProducts();

/* ── Scrapování vypnuto – používáme statická data ── */
let refreshing = false;
async function refreshAll() {
  console.log("[jaf] refresh skipped – používáme statická data");
}
...
function invoiceHTML(order, user) {
  ...
  const itemsRows = order.items.map((it) => {
    ...
    const line = unit * qty;
    const note = it.note ? `<br><span style="font-size:0.75rem;color:#666;">${it.note}</span>` : "";
    return `<tr>
      <td style="padding:8px 0;border-bottom:1px solid rgba(0,0,0,0.08);">${it.name}${note}</td>
